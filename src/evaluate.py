...
def recall_by_event_type(evaluated_events: dict, total_number_events: dict):
    clicks = 0
    carts = 0
    orders = 0
    for event in evaluated_events.values():
        if 'clicks' in event and event['clicks']:
            clicks += event['clicks']
        if 'carts' in event and event['carts']:
            carts += event['carts']
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test-labels', default="../test/resources/output_split/test_labels.jsonl", type=str)
    parser.add_argument('--predictions', default="../test/resources/predictions.csv", type=str)
